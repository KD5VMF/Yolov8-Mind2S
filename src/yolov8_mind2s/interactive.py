...
BANNER = r'''
__ __ ___ _ ___ __ __ _ _ ___ ___
...
def interactive_main() -> int:
    ...
    default_choice = str(next((i for i, d in enumerate(devs, start=1) if d.name == default_dev), 1))
    dev_in = prompt("Pick device number (or type a device string)", default_choice)
    ...
    model, _pred_dev = load_model(model_id, device=device, prefer_int8_on_npu=prefer_int8_on_npu, imgsz=imgsz)

    names: Dict[int, str] = get_class_names(model)
    n = len(names) if names else 80

    print()
    print("Classes (choose what to detect):")
    items = [f"{k+1:2d}:{names.get(k, str(k))}" for k in range(n)]
